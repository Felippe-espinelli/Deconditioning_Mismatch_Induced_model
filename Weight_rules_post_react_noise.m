function [weight_update] = Weight_rules_post_react(weight_update, nr_learning_rounds, t_initial, t_final, nr_neurons_h, patterns_h, Ix, decay, S, D, saturation,noise)
...
for ii = 1:nr_learning_rounds
    ...
    % shock and non shock
    term4=D*max(mismatch).*(postivemismatch'*SteadyState).*(weight_update<0);
%     term4=D*((mismatch.*postivemismatch)'*SteadyState).*(weight_update<-epsilon);
    ...
    dw0 = term1 + term2 + term3 + term4; 
    
    temp1 = find(dw0 > 0 & weight_update > 0); dw0(temp1) = dw0(temp1) .* (1 - weight_update(temp1)); 
    temp1 = find(dw0 < 0 & weight_update < 0); dw0(temp1) = dw0(temp1) .* (1 + weight_update(temp1)); 
    clear temp1
